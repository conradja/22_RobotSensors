...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_sensor(self, inches, speed):
        ...
        self.right_motor = Motor('C', motor_type='wheel')
        self.left_motor = Motor('B',motor_type='wheel' )
        self.right_motor.reset_position()
        self.left_motor.reset_position()
        left_position = 0
        right_position = 0
        target_position = (360 / (1.3*math.pi)) * inches
        if inches<0:
            speed = speed * -1
        while abs(left_position) < abs(target_position) and abs(right_position) < abs(target_position):
